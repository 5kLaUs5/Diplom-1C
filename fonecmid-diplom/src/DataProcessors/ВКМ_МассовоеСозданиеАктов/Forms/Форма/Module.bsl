...
Процедура ВКМ_ПериодПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.Период) И (Объект.Период.ДатаНачала <> НачалоМесяца(Объект.Период.ДатаНачала) ИЛИ Объект.Период.ДатаОкончания <> КонецМесяца(Объект.Период.ДатаОкончания)) Тогда
		
		Объект.ВКМ_Период.ДатаНачала = НачалоМесяца(Объект.ВКМ_Период.ДатаНачала);
		Объект.ВКМ_Период.ДатаОкончания = КонецМесяца(Объект.ВКМ_Период.ДатаОкончания);
	...
	Объект.ВКМ_СписокДоговоровИРеализаций.Очистить();
...
Функция СоздатьДокументыНаСервере()
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
		"Обработки.BKM_МассовоеСозданиеАктов.СоздатьСписокДокументовРеализацииТоваровИУслуг", Объект.Период.ДатаНачала,
